...
# 自定义台阶地形（长条形障碍）:
DOG3_OBSTACLE_TERRAIN_CFG = terrain_gen.TerrainGeneratorCfg(
    size=(8.0, 8.0),
    border_width=2.0,
    num_rows=10,
    num_cols=20,
...
@configclass
class ATDogDog3SandEnvCfg(LocomotionVelocityRoughEnvCfg):
    # 机身主刚体名称，用于:
    # 1) 传感器挂载（高度扫描器）
    ...
    def __post_init__(self):
        ...
        self.scene.terrain.terrain_generator = DOG3_OBSTACLE_TERRAIN_CFG
        ...
        if self.__class__.__name__ == "ATDogDog3SandEnvCfg":
