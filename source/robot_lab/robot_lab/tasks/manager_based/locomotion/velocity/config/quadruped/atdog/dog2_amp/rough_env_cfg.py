...
import robot_lab.tasks.manager_based.beyondmimic.mdp as mimic_mdp
...
DOG2_SLOPE_TERRAIN_CFG = terrain_gen.TerrainGeneratorCfg(
    size=(8.0, 8.0),
    border_width=2.0,
    num_rows=10,
    num_cols=20,
...
@configclass
class ATDogDog2AmpRoughEnvCfg(LocomotionVelocityRoughEnvCfg):
    # 机身主刚体名称，用于:
    # 1) 传感器挂载（高度扫描器）
    ...
    def __post_init__(self):
        ...
        # ------------------------------Scene 场景与传感器------------------------------
        # 覆写默认 rough terrain：使用连续缓坡地形（最大坡度 20°）
        self.scene.terrain.terrain_type = "generator"
        self.scene.terrain.terrain_generator = DOG2_SLOPE_TERRAIN_CFG
        ...
        # ------------------------------AMP imitation commands & rewards------------------------------
        # NOTE:
        # - This is the minimal runnable AMP-style extension on top of the velocity task.
        # - Motion file must follow beyondmimic format keys:
        #   joint_pos, joint_vel, body_pos_w, body_quat_w, body_lin_vel_w, body_ang_vel_w, fps.
        self.commands.motion = mimic_mdp.MotionCommandCfg(
            asset_name="robot",
            motion_file=os.path.join(os.path.dirname(__file__), "motions", "atdog2_vmc_walk_manager.npz"),
            ...
            # only resample at episode transitions by using a very large interval.
            resampling_time_range=(1.0e9, 1.0e9),
            pose_range={"x": (-0.05, 0.05), "y": (-0.05, 0.05), "z": (-0.01, 0.01), "roll": (-0.1, 0.1), "pitch": (-0.1, 0.1), "yaw": (-0.2, 0.2)},
            velocity_range={"x": (-0.5, 0.5), "y": (-0.5, 0.5), "z": (-0.2, 0.2), "roll": (-0.52, 0.52), "pitch": (-0.52, 0.52), "yaw": (-0.78, 0.78)},
            joint_position_range=(-0.1, 0.1),
            debug_vis=False,
        )
        self.rewards.motion_anchor_pos = RewTerm(
            func=mimic_mdp.motion_global_anchor_position_error_exp,
            weight=0.5,
            params={"command_name": "motion", "std": 0.3},
        )
        self.rewards.motion_anchor_ori = RewTerm(
        ...
        if self.__class__.__name__ == "ATDogDog2AmpRoughEnvCfg":
