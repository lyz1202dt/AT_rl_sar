...
# use local assets
from robot_lab.assets.atdog import AT_DOG3_ARM_CFG  # isort: skip
...
DOG3_ARM_ROUGH_TERRAIN_CFG = terrain_gen.TerrainGeneratorCfg(
    size=(8.0, 8.0),
    border_width=2.0,
    num_rows=10,
    num_cols=20,
...
@configclass
class ATDogDog3ArmRoughEnvCfg(LocomotionVelocityRoughEnvCfg):
    # 机身主刚体名称，用于:
    # 1) 传感器挂载（高度扫描器）
    ...
    def __post_init__(self):
        # 先继承父类默认配置，再按 ATDog Dog3ARM 粗糙地形任务覆写
        super().__post_init__()
        ...
        self.scene.robot = AT_DOG3_ARM_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
        ...
        if self.__class__.__name__ == "ATDogDog3ArmRoughEnvCfg":
