...
"""
Motion Replayer for ATDog2 Quadruped

This script replays motion data for the ATDog2 quadruped robot in Isaac Sim.

Usage:
    python motion_replayer.py [options]

Options:
    --motion MOTION_FILE    Motion data file to replay
    --device DEVICE       Device to run simulation on (default: cuda:0)

Example:
    # Replay a motion file
    python motion_replayer.py --motion atdog2_vmc_walk.npz
"""
...
# Find the index for the root body, typically 'base' for quadruped assets
try:
    print(f"Searching for 'base' in the following list of body names: {motion.body_names}")
    root_idx = motion.body_names.index("base")
    print(f"Found root body 'base' at index: {root_idx}")
except (ValueError, AttributeError):
    print("\nError: Could not find 'base' in the motion file's body_names.")
    print("The motion replayer requires 'base' to be defined as the root body.")
...
scene_cfg.robot = AT_DOG2_CFG.replace(prim_path="/World/Robot")
...
print("\nStarting ATDog2 motion replay loop...")
