...
from .atdog2_amp_env_cfg import ATDog2AmpWalkEnvCfg
from .motions import MotionLoader


class ATDog2AmpEnv(DirectRLEnv):
    cfg: ATDog2AmpWalkEnvCfg

    def __init__(self, cfg: ATDog2AmpWalkEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.command_resample_steps = max(1, int(self.cfg.command_resample_time_s / self.physics_dt))
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        resample_mask = (self.episode_length_buf[:, 0] % self.command_resample_steps) == 0
        resample_env_ids = torch.nonzero(resample_mask, as_tuple=False).squeeze(-1)
        if resample_env_ids.numel() > 0:
            self._sample_commands(resample_env_ids)
    ...
    def _get_observations(self) -> dict:
        ...
        base_ang_vel = self.robot.data.body_ang_vel_w[:, self.ref_body_index]
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # ================= total reward ==========================
        rew_track_lin, rew_track_yaw = compute_tracking_rewards(
            self.commands,
            self.robot.data.body_lin_vel_w[:, self.ref_body_index, :2],
            self.robot.data.body_ang_vel_w[:, self.ref_body_index, 2],
            self.cfg.rew_track_lin_vel_xy_exp,
            self.cfg.rew_track_ang_vel_z_exp,
            self.cfg.track_lin_vel_xy_std,
            self.cfg.track_ang_vel_z_std,
        )
        velocity_reward = rew_track_lin + rew_track_yaw
        total_reward = imitation_reward + basic_reward + velocity_reward
        ...
        log_dict = {
            ...
            "rew_track_lin_vel_xy": rew_track_lin.mean().item(),
            "rew_track_ang_vel_z": rew_track_yaw.mean().item(),
    ...
    def _reset_strategy_random(
        ...
        root_state[:, 2] += 0.03  # lift slightly to avoid collisions with the ground during reset
    ...
    def _sample_commands(self, env_ids: torch.Tensor):
        if env_ids.numel() == 0:
            return
        cmd_range = self.cfg.command_velocity_range
        n = env_ids.shape[0]
        lin_x = torch.empty(n, device=self.device).uniform_(cmd_range["lin_vel_x"][0], cmd_range["lin_vel_x"][1])
        lin_y = torch.empty(n, device=self.device).uniform_(cmd_range["lin_vel_y"][0], cmd_range["lin_vel_y"][1])
        ang_z = torch.empty(n, device=self.device).uniform_(cmd_range["ang_vel_z"][0], cmd_range["ang_vel_z"][1])
        self.commands[env_ids, 0] = lin_x
        self.commands[env_ids, 1] = lin_y
        self.commands[env_ids, 2] = ang_z
...
def compute_tracking_rewards(
    ...
    base_lin_vel_xy_w: torch.Tensor,
    base_ang_vel_z_w: torch.Tensor,
    rew_scale_track_lin_vel_xy_exp: float,
    rew_scale_track_ang_vel_z_exp: float,
    std_lin: float,
    std_yaw: float,
):
    lin_vel_error = torch.sum(torch.square(commands[:, :2] - base_lin_vel_xy_w), dim=1)
    ang_vel_error = torch.square(commands[:, 2] - base_ang_vel_z_w)
    rew_track_lin = rew_scale_track_lin_vel_xy_exp * torch.exp(-lin_vel_error / (std_lin * std_lin))
    rew_track_yaw = rew_scale_track_ang_vel_z_exp * torch.exp(-ang_vel_error / (std_yaw * std_yaw))
    return rew_track_lin, rew_track_yaw
